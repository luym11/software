...
from vision.camera_helpers import exact_to_normal, get_image_scale, get_option_size_int
...
#vision_options = [ gui_options.IntOption('block_size', 67, 0, 100),
 #                  gui_options.IntOption('c_thresh', -13, -100, 100),
  #                 gui_options.IntOption('erode_size', 18, 0, 100),
   #                gui_options.IntOption('dilate_size', 38, 0, 100),
    #               gui_options.DoubleOption('min_percent_frame', 0.2, 0, 100),
     #              gui_options.DoubleOption('min_rectangularity', 1000),
      #             gui_options.DoubleOption('heuristic_power', 5),
...
vision_options = [ 
                   gui_options.IntOption('block_size', 34, 0, 100),
                   gui_options.IntOption('c_thresh', -13, -100, 100),
                   gui_options.IntOption('erode_size', 18, 0, 100),
                   gui_options.IntOption('dilate_size', 38, 0, 100),
                   gui_options.DoubleOption('min_percent_frame', 0.2, 0, 100),
...
class Pipes(ModuleBase):
    def process(self, mat):
        ...
        image_scale = get_image_scale(mat.shape[1], "downward")

        block_size = get_option_size_int(self.options['block_size'], 15, image_scale, True)
        erode_size = get_option_size_int(self.options['erode_size'], 1, image_scale, True)
        dilate_size = get_option_size_int(self.options['dilate_size'], 1, image_scale, True)

        lab_image = cv2.cvtColor(mat, cv2.COLOR_BGR2LAB)
        lab_split = cv2.split(lab_image)

        minPercentScale = 1/1000 * image_scale
        ...
        for c in contours:
            ...
            if area < image_size * self.options['min_percent_frame'] * minPercentScale:
                continue
        ...
        pipe_center = exact_to_normal(best.center[0], best.center[1], mat.shape[1])
