...
class Buoys(ModuleBase):
    ...
    def process(self, mat):
        ...
        def morph_and_find_contours(channels_and_names, channel_set_name,
                                    debug=False, debug_channel_names=[]):
            """
            channels_and_names: [(channel, name), ...] - The mat will be be thresholded according to the parameters identified by `name` and `channel_set_name`.
            channel_set_name: Name of the channel set (prepend to all posts)
            debug: Enable debugging output (posting images)
            debug_channel_names: Name of each channel to identify each post
            """
            # Apply thresholds for segmentation
            channels_and_bounds = starmap(lambda channel, name: (channel,
            ...
            if debug:
                for thresh, channel_name in zip(thresheds, chain(debug_channel_names, repeat(""))):
