...
class ModuleBase:
    ...
    # TODO replace these shims after final normalized coordinates API is decided
    def normalized(self, coord, mat=None, axis=None):
        if mat is None:
            mat = self._next_images[0]

        def norm(compon, axis):
            axis_len = mat.shape[axis]
            return (compon - axis_len / 2) / mat.shape[1] * 2
    ...
    def denormalized(self, coord, mat=None, round=False):
        ...
        def denorm(compon, axis=None):
            axis_len = mat.shape[axis]
            denormed = compon / 2 * mat.shape[1] + axis_len / 2
            if round:
