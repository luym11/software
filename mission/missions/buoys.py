...
class LocateAdjacentBuoy(Task):
    ...
    def __init__(self, validator, right=True, changeDir=True, *args, **kwargs):
        ...
        self.task_classes = []
        if changeDir:
            self.task_classes.append(lambda: LocateBuoyStrafe(validator, right= right))
...
class LocateAlignedBuoy(Task):
    ...
    def __init__(self, validator, forward=True, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        self.validator = validator
        self.task_classes = [lambda: LocateBuoySurge(validator, forward=forward)]
...
class BuoyAlign(Task):
    def __init__(self, location_validator, target_coordinates, vision_group,
            collision_validator, ram_concurrent_task=NoOp(), first_buoy=False, right=True, yellow=False, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.logv("Starting {} task".format(self.__class__.__name__))
        ...
        if first_buoy:
            self.locator_task_a = LocateAlignedBuoy(self.location_validator, forward=True)
        else:
            self.locator_task_a = LocateAdjacentBuoy(self.location_validator, right=right)
        self.locator_task_b = LocateAlignedBuoy(self.location_validator, forward=False)
...
class Buoy(Task):
    ...
    buoy
    """
    def __init__(self, buoy, right=True, first_buoy=False, yellow=False, align_only=False, *args, **kwargs):
        super().__init__(*args, **kwargs)
        ...
        if align_only:
            ...
            self.ram_task = BuoyAlign(self.location_validator,
                (self.buoy.r_side_x.get, self.buoy.center_y.get), self.buoy,
                self.collision_validator, self.align_task, right=right, first_buoy=first_buoy, yellow=yellow)

        elif yellow:
            self.align_task = AlignTarget(self.location_validator,
                LocateAdjacentBuoy(self.location_validator, right=right),
                (self.buoy.r_side_x.get, self.buoy.bottom_y.get), self.buoy, self.heading_task)
...
class AllBuoys(Task):
    ...
    def on_first_run(self):
        ...
        self.task = Sequential(
            Depth(constants.BUOY_SEARCH_DEPTH),
            Conditional(MasterConcurrent(Sequential(
                    self.heading_task,
                    #Depth(0.9, error=.01)
                    ...
                    Buoy(red_buoy_results, first_buoy=True, right=redRight()),
                    #self.depth_task,
                    ...
                    SeqLog("Looking for green buoy"), LocateBuoyStrafe(lambda: location_validator(green_buoy_results), right=greenRight(), timeout=3),
                    Buoy(green_buoy_results, right=greenRight()),
                    #self.depth_task,
                    SeqLog("Looking for yellow buoy"), LocateBuoyStrafe(lambda: location_validator(yellow_buoy_results), right=yellowRight(), timeout=2),
                    Buoy(yellow_buoy_results,  right=yellowRight(), yellow=True),

                    Log("re-aligning red buoy"), LocateBuoyStrafe(lambda: location_validator(red_buoy_results), right= secondRedRight(), timeout=2),
                    Buoy(red_buoy_results,  right=secondRedRight(), yellow=False, align_only=True),
                ),
                ...
                on_fail=Sequential(
                    Zero(),
                    self.heading_task,
                    SeqLog("Going to Over depth"), self.up_task,
                    SeqLog("Going over buoys"), self.over_task,
                    Timed(VelocityX(.4),8),
            )
        ))
...
class RetryBuoys(Task):
    ...
    def on_first_run(self):
        ...
        self.task = Sequential(
            ...
            #self.depth_task,
            #SeqLog("Looking for green buoy stage 1"), LocateBuoyStrafe(lambda: location_validator(yellow_buoy_results), right=True, timeout=3),
            SeqLog("Looking for green buoy"), LocateBuoyStrafe(lambda: location_validator(green_buoy_results), right=not greenRight(), timeout=3),
            Buoy(green_buoy_results, right=not greenRight()),
            #self.depth_task,
            SeqLog("Looking for yellow buoy"), LocateBuoyStrafe(lambda: location_validator(yellow_buoy_results), right=not yellowRight(), timeout=2),
            Buoy(yellow_buoy_results,  right=not yellowRight(), yellow=True),
            HeadingRestore(),

            SeqLog("Rising to Over depth"), self.up_task,
            SeqLog("Going around buoys"), self.over_task
        )
    def on_run(self):
        if self.task.finished:
            self.finish()
